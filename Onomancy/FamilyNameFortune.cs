...
namespace Onomancy
{
    ...
    /// 天格クラス
    /// </summary>
    public class FamlyNameFortune : FortuneBase, IDisplay
    {
        /// <summary>
        /// コンストラクタ
        /// </summary>
        ...
        public FamlyNameFortune(string fotuneName, List<NameBase> names) : base(fotuneName)
        {
        ...
        {
            this.Stroke = names[0].TotalStroke;
        }
        ...
        public void Display(List<NameBase> names)
        {
            for (int i = 0; i < names[0].Value.Length + names[1].Value.Length + 1; i++)
            {
                ...
                else if (i > 0 && i < names[0].Value.Length - 1)
                {
                    Console.Write("─");
                }
                else
